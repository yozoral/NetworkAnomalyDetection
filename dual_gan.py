# -*- coding: utf-8 -*-
# @Author: Jingyi
# @Date:   2020-09-08 21:31:37
# @Last Modified by:   jingyi
# @Last Modified time: 2020-09-10 00:27:10
import argparse
import os
import numpy as np
import math
...
for epoch in range(opt.n_epochs):
    for i, batch in enumerate(train_loader):
        ...
        input_features = batch_features.view(-1, 1, 43)
        # import pdb; pdb.set_trace()

        latent_features = torch.from_numpy(np.random.normal(size=[batch_size, 1, 43])).float().cuda()

        imgs_A = input_features
        imgs_B = latent_features
